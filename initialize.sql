...
\copy Payments(transactionID, amount, timestamp, method, note, payer_userID, subscriptionID, payer_ad_accountID) FROM data/payments.csv csv header; -- add one more attr!; deleted tID (serialized); un-serialized
...
\copy Posts(postID, IP, timestamp, title, content, original_content_tag, spoiler_tag, NSFW_tag, poster_userID, flairID, subredID, popularity) FROM data/posts.csv csv header; -- serialized ID; unserialized
\copy AdvertisingMaterials(ad_materialID, status, impressions, eCPM, click, CPC, CTR, accountID, transactionID) FROM data/ad_materials.csv csv header; -- serialized ID, normalized (deleted amount_spent); unserialzied
...
\copy Interactions(interactionID,type, postID, done_by_userID, comID) FROM data/interactions.csv csv header; -- delete id (it's serialized now)
